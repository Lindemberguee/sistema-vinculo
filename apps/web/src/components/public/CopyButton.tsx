...
export function CopyButton({ text, label = "Copiar código" }: { text: string; label?: string }) {
  ...
  return (
    <button
      type="button"
      onClick={copy}
      className="inline-flex items-center gap-1.5 rounded-full border border-line-strong bg-surface px-3 py-1.5 text-xs font-medium text-ink transition-colors hover:bg-canvas"
    >
