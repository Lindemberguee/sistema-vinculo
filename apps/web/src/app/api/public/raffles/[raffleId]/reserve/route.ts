...
import { reserveRaffleTickets } from "@/server/raffle/reserve";
...
const bodySchema = z
  .object({
    numbers: z.array(z.number().int().positive()).max(200).optional(),
  ...
  .refine((b) => (b.numbers && b.numbers.length) || b.quantity, {
    message: "Informe os números ou a quantidade",
  });
...
export async function POST(req: Request, ctx: { params: Promise<{ raffleId: string }> }) {
  ...
  try {
    const { donation, numbers } = await reserveRaffleTickets({
    ...
    console.error("reserveRaffleTickets failed", err);
